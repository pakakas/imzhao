...
An invoke block (tool call) is a **special grid**. Unlike normal grids which start with the standard grid marker `░`, the invoke special grid starts with invoke marker `¡`.
